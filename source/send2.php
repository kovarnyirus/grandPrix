...
if (!$errors) {
    ...
    $header .= 'From: =?utf-8?b?'.base64_encode("Заявка на обратный звонок [$email]").'?= <no-reply@'.$_SERVER['HTTP_HOST'].'>'."\r\n";
    $header .= 'Заявка на обратный звонок ['.$email.']';

    $subject = 'Заявка на обратный звонок ['.$email.']';
    ...
    $msg .= ' </head>'."\n";
    $msg .= ' <body>'."\n";
    $msg .= ' <h2>Заявка на обратный звонок</h2>'."\n";
